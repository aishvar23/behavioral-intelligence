...
const RULES: RuleDef[] = [
  { fn: x => x + 5,       label: 'f(x) = x + 5',     testValues: [1,2,3,4,5,6], challenge: 8  }, // → 13
  { fn: x => x * 2,       label: 'f(x) = x × 2',     testValues: [1,2,3,4,5,6], challenge: 9  }, // → 18
  { fn: x => x * 2 + 3,   label: 'f(x) = 2x + 3',    testValues: [1,2,3,4,5,6], challenge: 7  }, // → 17
  { fn: x => 15 - x,      label: 'f(x) = 15 − x',    testValues: [1,2,3,4,5,6], challenge: 8  }, // → 7
  { fn: x => x * x,       label: 'f(x) = x²',        testValues: [1,2,3,4,5,6], challenge: 7  }, // → 49
  { fn: x => x * x + 1,   label: 'f(x) = x² + 1',    testValues: [1,2,3,4,5,6], challenge: 5  }, // → 26
  { fn: x => x * (x + 1), label: 'f(x) = x×(x+1)',   testValues: [1,2,3,4,5,6], challenge: 6  }, // → 42
  { fn: x => x * x - x,   label: 'f(x) = x² − x',    testValues: [2,3,4,5,6,7], challenge: 8  }, // → 56
];
...
export default function BlackBoxGame({ sessionId, onComplete }: Props) {
  ...
  const rule = RULES[round % RULES.length];
  ...
  useEffect(() => {
    ...
    timerInterval.current = setInterval(() => {
      ...
      if (pct === 0) {
        ...
        setSubmitted(true);
        setWasCorrect(false);
        setTimeout(() => advance(round + 1), 1200);
      }
    }, 80);
  ...
  function handleTest(val: number) {
    if (submitted) return;
    if (tested.some(p => p.input === val)) return; // already tested
  ...
  function handleSubmit() {
    ...
    // Scoring: fewer tests = higher potential, correct = points, time bonus
    ...
    setTimeout(() => advance(round + 1), 1500);
  }
  ...
  const currentRule = RULES[round % RULES.length];
